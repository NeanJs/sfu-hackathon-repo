...
export default function OnboardingPage() {
          ...
          onClick: () => {
            if (companyList) {
              if (typeof window !== "undefined") {
                sessionStorage.setItem("leadger_companyList", JSON.stringify(companyList));
              }
            }
            router.push("/demo");
          },
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900">
        ...
        <section className="flex min-h-screen items-center justify-center px-6">
          <div className="max-w-3xl text-center">
            ...
            <p className="mt-6 text-white/90 text-lg sm:text-xl font-medium max-w-2xl mx-auto leading-relaxed">
              Answer a few quick questions so we can personalize company
              insights to your values and preferences.
            </p>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link
                href="/"
                className="rounded-full border-2 border-white/40 text-white px-8 py-4 text-lg font-semibold hover:bg-white/10 hover:border-white/60 transition-all duration-200 backdrop-blur-sm"
              >
                Skip for now
              </Link>
            </div>
            ...
            <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
              <div className="group rounded-3xl border border-white/30 bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl p-6 hover:bg-white/20 transition-all duration-300 hover:scale-105">
                ...
                <div className="text-white/85 text-sm leading-relaxed">
                  Tailored company lists matched to your values and preferences.
                </div>
              </div>
              <div className="group rounded-3xl border border-white/30 bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl p-6 hover:bg-white/20 transition-all duration-300 hover:scale-105">
                <div className="flex items-center gap-3 mb-3">
                ...
                <div className="text-white/85 text-sm leading-relaxed">
                  Just a few quick questions to get started - no lengthy forms.
                </div>
              </div>
              <div className="group rounded-3xl border border-white/30 bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl p-6 hover:bg-white/20 transition-all duration-300 hover:scale-105">
                <div className="flex items-center gap-3 mb-3">
                ...
                <div className="text-white/85 text-sm leading-relaxed">
                  Your preferences help personalize your feed while staying
                  private.
                </div>
              </div>
            </div>
        ...
        <section className="relative min-h-screen px-4 pt-20 pb-32">
          <div className="mx-auto max-w-3xl">
            ...
            <div className="rounded-3xl border border-white/20 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl p-6 sm:p-8 shadow-2xl">
              <UserForm
                formId="onboarding"
                onSubmitted={async (_values: any, apiResponse?: any) => {
                  // Try to get companyList from API response if available
                  if (apiResponse && apiResponse.companyList) {
                    setCompanyList(apiResponse.companyList);
                  }
                  setSubmitted(true);
                  setStep("done");
                  window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      {step === "done" && (
        <section className="flex flex-col min-h-screen items-center justify-center px-6">
          <div className="max-w-2xl text-center mb-10">
            ...
            <p className="mt-6 text-white/90 text-lg sm:text-xl font-medium max-w-xl mx-auto leading-relaxed mb-12">
              We've saved your preferences. Here are companies tailored to your
          ...
          <div className="w-full max-w-5xl">
            ...
            companyList.companies.length > 0 ? (
              <CompanyDirectory
                selectable={false}
                onCompanyOpen={() => {}}
                // Pass the personalized companies directly
                initialCompanies={companyList.companies}
                initialTitle={companyList.discovery_list_title}
              />
            ) : (
              <div className="text-white/80 text-center py-12">
                No personalized companies found.
              </div>
            )}
          </div>
        </section>
